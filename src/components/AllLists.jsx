...
import { removeListAction, setActiveList } from '../redux/actions/listsAction';
...
function AllLists({ items }) {
  ...
  return (
    <div className="allLists">
      ...
      {items &&
        items.map(({ name, colors, id }, index) => (
          <div
            key={`${name}_${index}`}
            onClick={() => activeList(id, index)}
            className={classNames('listItem', {
              active: id === activeItem,
            })}>
            <div className={`listItem__circle ${colors.color}`}></div>
            <div className="listItem__name">
              <span>{name}</span>
            </div>
            <div onClick={() => removeList(id)} className="removeIcon">
              <i>
                <svg
                  width="10"
                  height="10"
